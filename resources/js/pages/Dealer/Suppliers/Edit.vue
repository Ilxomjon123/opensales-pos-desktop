...
const props = defineProps<{ supplier: { data: Supplier } }>();
...
function submit() {
    form.put(`/dealer/suppliers/${supplier.value.id}`);
}
...
<template>
    <Head :title="t('pageDealer.suppliers.editHead')" />

    <div class="flex flex-col gap-4 p-4 sm:gap-6 sm:p-6">
        <div class="flex items-center gap-2">
            <Button variant="ghost" size="icon" @click="router.get(`/dealer/suppliers/${supplier.id}`)">
                <ArrowLeft class="h-4 w-4" />
            </Button>
            <h1 class="text-xl font-bold tracking-tight sm:text-2xl">{{ supplier.name }}</h1>
        </div>
            ...
            <CardContent class="pt-6">
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <Label for="is_active" class="cursor-pointer">{{ t('pageDealer.suppliers.isActive') }}</Label>
                    </div>

                    <div class="flex justify-end gap-2 pt-2">
                        <Button variant="outline" type="button" @click="router.get(`/dealer/suppliers/${supplier.id}`)">{{ t('pageDealer.suppliers.cancel') }}</Button>
