...
const props = defineProps<{
    deliveryman: { data: { id: number; name: string; username: string; phone: string | null } };
}>();

const d = props.deliveryman.data;
...
function submit() {
    form.put(`/dealer/deliverymen/${d.id}`);
}
...
<template>
    <Head :title="`${d.name} — ${t('pageDealer.deliverymen.editTitleSuffix')}`" />
    ...
    <div class="mx-auto w-full max-w-4xl p-4 md:p-8">
        <div class="mb-6 flex items-center gap-3 sm:mb-8 sm:gap-4">
            ...
            <div class="min-w-0">
                ...
                <p class="text-sm text-muted-foreground">{{ t('pageDealer.deliverymen.editSubtitle') }}</p>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                {{ t('pageDealer.deliverymen.editPersonalInfoDesc') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-1">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                {{ t('pageDealer.deliverymen.changePasswordDesc') }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="sticky bottom-0 z-10 -mx-4 flex items-center justify-end gap-3 border-t bg-background/80 px-4 py-4 backdrop-blur md:-mx-8 md:px-8">
                <Button variant="outline" type="button" @click="router.get('/dealer/deliverymen')">{{ t('pageDealer.deliverymen.cancel') }}</Button>
                <Button type="submit" :disabled="form.processing" class="min-w-[140px]">
                    <Spinner v-if="form.processing" class="mr-2" />
                    {{ form.processing ? t('pageDealer.deliverymen.saving') : t('pageDealer.deliverymen.saveBtn') }}
                </Button>
            </div>
        </form>
    </div>
