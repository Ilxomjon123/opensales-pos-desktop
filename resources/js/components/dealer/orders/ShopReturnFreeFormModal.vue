...
function addProduct() {
    ...
    if (rows.some((r) => r.key === key)) {
        error.value = t('pageDealer.orders.alreadyInListProduct');
...
function clamp(r: Row) {
    if (r.pack_size > 1) {
        r.return_pack_qty = Math.max(0, Number(r.return_pack_qty) || 0);
        r.return_unit_qty = r.bulk_only
            ? 0
            : Math.max(0, Number(r.return_unit_qty) || 0);
    } else {
        r.return_unit_qty = Math.max(0, Number(r.return_unit_qty) || 0);
        r.return_pack_qty = 0;
    }
}
...
const paidCash = computed<number>({
    get: () => {
        ...
        if (mode === 'all-cash') {
            return Math.max(0, refundTotal.value);
        }
...
const previousBalance = computed(() => props.shopBalance ?? 0);
...
function submit() {
        ...
        .map((r) => ({
            ...
            pack_qty:
                r.pack_size > 1
                    ...
                    : null,
            unit_cost: Math.max(0, r.unit_cost),
            pack_unit_cost: r.pack_unit_cost,
            disposition: 'restock',
        }));

    if (items.length === 0) {
        error.value = t('pageDealer.orders.atLeastOneRow');
    ...
    router.post(
        `/dealer/shops/${props.shopId}/return`,
        {
            reason: reason.value,
...
<template>
    <Dialog :open="open" @update:open="(v) => emit('update:open', v)">
        <DialogContent
            class="flex h-[100dvh] max-h-[100dvh] w-screen max-w-none flex-col gap-0 rounded-none border-0 p-0 sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:w-auto sm:max-w-3xl sm:rounded-lg sm:border lg:max-w-4xl"
        ...
        >
            <DialogHeader class="border-b px-3 py-2 sm:px-4 sm:py-3">
                <DialogTitle class="pr-8 text-sm font-semibold sm:text-base">
                    {{ t('pageDealer.orders.shopReturnFree') }}
                    <span
                        v-if="shopName"
            ...
            <div class="flex-1 space-y-2.5 overflow-y-auto p-3 sm:p-4">
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <Input
                        v-model="note"
                        class="h-9"
                        :placeholder="t('pageDealer.orders.noteOptional')"
                    />
                </div>
                ...
                <div class="flex gap-2">
                    <div class="flex-1">
                        <SearchableSelect
                            ...
                            label-key="name"
                            :placeholder="
                                t('pageDealer.orders.addProductPlaceholder')
                            "
                            :search-placeholder="
                                t('pageDealer.orders.productNamePlaceholder')
                            "
                ...
                >
                    {{ t('pageDealer.orders.selectProductForReturn') }}
                </div>
            ...
            <div class="border-t bg-muted/30 px-3 py-2.5 sm:px-4 sm:py-3">
                ...
                <div class="mt-2 grid grid-cols-2 gap-2">
                    <Input
                        ...
                        class="h-9"
                        :placeholder="t('pageDealer.orders.cash')"
                    />
                    <Input
                        id="return_paid_card"
                        type="number"
                        ...
                        class="h-9"
                        :placeholder="t('pageDealer.orders.card')"
                    />
                </div>
                ...
                <Input
                    v-if="paidCard > 0"
                    id="return_cardholder"
                    type="text"
                    v-model="cardholderName"
                    :placeholder="t('pageDealer.orders.cardholderPlaceholder')"
                    class="mt-2 h-9"
                />
                ...
                >
                    {{
                        processing
                            ...
                            : t('pageDealer.orders.writeReturn')
                    }}
                </Button>
            </DialogFooter>
        </DialogContent>
