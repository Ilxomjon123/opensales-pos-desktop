...
const suppliersSortedByDebt = computed<SupplierOption[]>(() =>
    [...props.suppliers].sort(
        (a, b) => Math.abs(b.balance) - Math.abs(a.balance),
    ),
);

const selectedSupplier = computed<SupplierOption | null>(() => {
    if (!form.supplier_id) {
        return null;
    }

    return (
        props.suppliers.find((s) => String(s.id) === form.supplier_id) ?? null
    );
});

const newBalance = computed<number | null>(() => {
    if (!selectedSupplier.value || !form.amount) {
        return null;
    }

    const delta = form.type === 'credit' ? form.amount : -form.amount;

    return selectedSupplier.value.balance + delta;
});
        ...
        <DialogContent class="max-w-lg">
            <DialogHeader>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <DialogTitle>{{
                            t('pageDealer.finance.supplierPaymentTitle')
                        }}</DialogTitle>
                        <DialogDescription class="mt-0.5">
                            {{ t('pageDealer.finance.supplierPaymentDesc') }}
                        </DialogDescription>
                    </div>
                </div>
            </DialogHeader>
            ...
            <form @submit.prevent="submit" class="space-y-5 pt-2">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2 rounded-lg border p-1">
                        ...
                        >
                            ...
                            {{ t('pageDealer.finance.makePayment') }}
                        </button>
                        <button
                            type="button"
                            class="flex items-center justify-center gap-2 rounded-md py-2.5 text-sm font-medium transition-colors"
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        <span v-if="form.type === 'credit'">{{
                            t('pageDealer.finance.creditHint')
                        }}</span>
                        <span v-else>{{
                            t('pageDealer.finance.debitHint')
                        }}</span>
                    </p>
                </div>
                ...
                <div>
                    <Label class="mb-2 flex items-center gap-1.5">
                        ...
                        {{ t('pageDealer.finance.supplier') }}
                        <span class="text-destructive">*</span>
                    </Label>
                    <SearchableSelect
                        v-model="form.supplier_id"
                        :items="suppliersSortedByDebt"
                        value-key="id"
                        label-key="name"
                        :placeholder="t('pageDealer.finance.selectSupplier')"
                        :search-placeholder="
                            t('pageDealer.finance.supplierNamePlaceholder')
                        "
                        :empty-text="t('pageDealer.finance.supplierNotFound')"
                    >
                ...
                <!-- To'lov turi (faqat To'lov berish uchun) -->
                ...
                <DialogFooter class="gap-2 pt-2">
                    ...
                    <Button
                        type="submit"
                        :disabled="
                            form.processing || !form.supplier_id || !form.amount
                        "
                        class="min-w-[120px]"
                    >
                        {{
